...
async function listScpsForTarget(targetId, client, filter = "SERVICE_CONTROL_POLICY") {
    try {
...
async function runExample() {
    const accessKeyId = 'ASIAYXWBOFS7VIKSFFG2'; // Replace
    const secretAccessKey = 'X+Jw5hcWB8SLlv1Ujsgz1ZCJvO1TztDnvxT6M+WJ'; // Replace
    ...
    try {
        ...
        const networkPerimeterPolicy = allScps.find(policy => policy.Name === "Network_Perimeter_1")

        if (!networkPerimeterPolicy) {
            console.error("Could not find the 'Network_Perimeter_1' policy.")
            return
        }
