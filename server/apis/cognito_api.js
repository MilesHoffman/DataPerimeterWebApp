...
async function authenticateUser(username, password) {
	...
	try {
		...
		if (response.ChallengeName === 'NEW_PASSWORD_REQUIRED') {
			...
			const newPasswordResponse = await cognitoIdentityProviderClient.send(respondToAuthChallengeCommand)
			...
			const respondToAuthChallengeCommand = new RespondToAuthChallengeCommand(respondToAuthChallengeParams)
			const passwordVerifierResponse = await cognitoIdentityProviderClient.send(respondToAuthChallengeCommand)
			...
			return {
				...
				refreshToken: passwordVerifierResponse.AuthenticationResult.RefreshToken,
			}
		}
...
async function main() {
	...
	try {
		...
		const getIdParams = {
			...
			Logins: {
				[`cognito-idp.${REGION}.amazonaws.com/${poolData.UserPoolId}`]: tokens.idToken,
			},
		}
